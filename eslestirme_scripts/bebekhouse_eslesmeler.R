...
ebebek_tables <- c("ebebek_bezmendil", "ebebek_beslenme", "ebebek_banyo", "ebebek_oyuncak", "ebebek_bebekodasi", "ebebek_aracgerec")
civil_tables <- c("bebekhouse_bezmendil", "bebekhouse_beslenme", "bebekhouse_banyo", "bebekhouse_oyuncak", "bebekhouse_bebekodasi", "bebekhouse_aracgerec")
...
for (i in 1:length(ebebek_tables)) {
  ...
  results_df <- data.frame(
    ...
    bebekhouse = texts2[best_match_indices[valid_match_indices]],
    bebekhousefiyat = price2[best_match_indices[valid_match_indices]],
    similarityscorebebekhouse = best_match_scores[valid_match_indices]
  )

  # Veritabanına sonuçları yaz
  results_table_name <- paste("ebebek_bebekhouse", sub("ebebek_", "", ebebek_tables[i]), sep = "_")
