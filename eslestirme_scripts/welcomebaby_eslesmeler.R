...
ebebek_tables <- c("ebebek_beslenme", "ebebek_banyo", "ebebek_oyuncak", "ebebek_bebekodasi", "ebebek_aracgerec", "ebebek_guvenlik", "ebebek_emzirme")
civil_tables <- c("welcomebaby_beslenme", "welcomebaby_banyobakim", "welcomebaby_oyuncak", "welcomebaby_bebekodasi", "welcomebaby_aracgerec", "welcomebaby_guvenlik", "welcomebaby_emzirme")


# Yardımcı fonksiyonlar. Markaları eşitliyoruz
extract_brand <- function(text) {
  strsplit(text, " ")[[1]][1]
}

# Ürünlerin içerdiği rakamları eşitliyoruz
...
for (i in 1:length(ebebek_tables)) {
  ...
  for (j in 1:length(texts1)) {
    ...
    for (k in 1:length(texts2)) {
      ...
      if (brand1 == brand2 && numbers1 == numbers2) {
        score <- stringsim(texts1[j], texts2[k], method = "jaccard") # Jaccard metriği ile eşleştir.
  ...
  # Sonuçları hazırla
  best_matches <- ifelse(is.na(best_match_indices) | best_match_scores < 0.3, NA, texts2[best_match_indices]) # 0.3 altındaki eşleşmeleri direkt at
  best_match_prices <- ifelse(is.na(best_match_indices) | best_match_scores < 0.3, NA, price2[best_match_indices]) # 0.3 altındaki eşleşmeleri direkt at
  ...
  results_df <- data.frame(
    ...
    welcomebaby = texts2[best_match_indices[valid_match_indices]],
    welcomebabyfiyat= price2[best_match_indices[valid_match_indices]],
    similarityscorewelcomebaby = best_match_scores[valid_match_indices]
  )

  # Veritabanına sonuçları yaz
  results_table_name <- paste("ebebek_welcomebaby", sub("ebebek_", "", ebebek_tables[i]), sep = "_")
