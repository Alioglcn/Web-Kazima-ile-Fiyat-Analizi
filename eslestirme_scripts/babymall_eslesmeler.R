...
civil_tables <- c("babymall_bezmendil", "babymall_beslenme", "babymall_guvenlik", "babymall_hamile", "babymall_banyo", "babymall_oyuncak", "babymall_bebekodasi", "babymall_aracgerec")


# Markaları eşitliyoruz.
extract_brand <- function(text) {
  strsplit(text, " ")[[1]][1]
}

#Ürünlerin içerdiği sayıları eşitliyoruz
...
for (i in 1:length(ebebek_tables)) {
  ...
  for (j in 1:length(texts1)) {
    ...
    for (k in 1:length(texts2)) {
      ...
      if (brand1 == brand2 && numbers1 == numbers2) {
        score <- stringsim(texts1[j], texts2[k], method = "jaccard") # Jaccard ile eşleştirme
  ...
  # Sonuçları hazırla
  best_matches <- ifelse(is.na(best_match_indices) | best_match_scores < 0.3, NA, texts2[best_match_indices]) # Benzerlik skoru 0.3 altındakileri direkt at
  ...
  results_df <- data.frame(
    ...
    babymall = texts2[best_match_indices[valid_match_indices]],
    babymallfiyat = price2[best_match_indices[valid_match_indices]],
    similarityscorebabymall = best_match_scores[valid_match_indices]
  )

  # Veritabanına sonuçları yaz
  results_table_name <- paste("ebebek_babymall", sub("ebebek_", "", ebebek_tables[i]), sep = "_")
