...
ui <- fluidPage(
  # titlePanel("Ürün Fiyatları Arasındaki Farklar"),
  br(),
  br(),
  ...
  #
  # fluidRow(column(6, h3("Eşleşen Ürünler"),
  #                 br(),
  #                 reactableOutput("veriTablosu")),
  #          column(6, highchartOutput("veriHighchart"))),
  #
  # fluidRow(h3("Ürünler Arasındaki Yüzdesel Fiyat Farkları"),
  #          br(),
  #          plotOutput("violinPlot", height = "500px")),
  #
  # fluidRow(highchartOutput("Numofmatches"))
...
server <- function(input, output, session) {
  ...
  banyobakim <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/BANYOBAKIM.xlsx")
  bebekodasi <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/BEBEKODASI.xlsx")
  emzirme <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/EMZIRME.xlsx")
  bez <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/BEZ.xlsx")
  guvenlik <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/GUVENLIK.xlsx")
  beslenme <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/BESLENME.xlsx")
  oyuncak <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/OYUNCAK.xlsx")
  aracgerec <- read_excel("/Users/aliogulcanet/Desktop/Fiyat Analizi/WebKazımaileFiyatAnalizi/uygulama_son_hali/ARAC_GEREC.xlsx")
  ...
  tum_veri_kategori <- rbindlist(kategoriler, idcol = "Kategori", fill = TRUE)
  ...
  # num_of_obs <- veri_farklar %>%
  #   summarize(
  #     ebebek_count = sum(!is.na(ebebek)))

  firma_sayisi <-reactive({
    selected_data() %>%
      group_by(Kategori, Firma) %>%
      summarize(Firma_Sayisi = n_distinct(Fiyat_Fark)) %>%
      # mutate(Firma_yuzdelik = (Firma_Sayisi / 1314) * 100) %>%
  ...
  output$Numofmatches <- renderHighchart({
    ...
    highchart() %>%
      hc_chart(type = "bar") %>%
      hc_title(text = "Her Kategoriye Göre Farklı Firmalar İle Eşleşen Ürün Oranları",
               style = list(fontWeight = "bold")) %>%
      hc_xAxis(categories = firma_sayisi$Firma,
               style = list(fontSize = "16px")) %>%
      hc_yAxis(
        title = list(
          text = "Eşleşen Ürün Oranları",
  ...
  data_pivot <- reactive({
    selected_product() %>%
      ...
      pivot_longer(cols = c(ebebek, civil, welcomebaby, babymall, madrenino, bebekhouse),
                   names_to = "Firma",
                   values_to = "Ürünler") %>%
      mutate(Firma = ifelse(Firma == "ebebek", "ebebek",
      ...
      Fiyatlar = case_when(
        Firma == "ebebek" & !is.na(ebebekfiyat) ~ paste0(ebebekfiyat, " TL"),
